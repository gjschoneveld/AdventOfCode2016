...
namespace Day22
{
    ...
    class Node
    {
        ...
        {
            string pattern = @"^/dev/grid/node-x(?<x>\d+)-y(?<y>\d+)\s*(?<size>\d+)T\s*(?<used>\d+)T\s*(?<avail>\d+)T\s*(?<use>\d+)%$";
            Regex cmdRegex = new Regex(pattern);
            Match cmdMatch = cmdRegex.Match(x);

            return new Node
            {
                x = int.Parse(cmdMatch.Groups["x"].Value),
                y = int.Parse(cmdMatch.Groups["y"].Value),
                used = int.Parse(cmdMatch.Groups["used"].Value),
                avail = int.Parse(cmdMatch.Groups["avail"].Value)
            };
        }
    }
