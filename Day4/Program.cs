...
namespace Day4
{
    class Room
    {
        ...
        {
            var letters = name.Where(c => char.IsLetter(c));
            var groupsOfLetters = letters.GroupBy(c => c);
            var sortedGroups = groupsOfLetters.OrderByDescending(g => g.Count()).ThenBy(g => g.Key);
            var checksumCharacters = sortedGroups.Take(checksumLength).Select(g => g.Key);

            var calculated = new string(checksumCharacters.ToArray());
        ...
        internal string DecryptName()
        {
            ...
            foreach (char letter in name)
            {
                ...
                {
                    char decodedLetter = (char)((letter - 'a' + id) % totalLetters + 'a');
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var roomString in input)
            {
                ...
                Match match = rgx.Match(roomString);
                ...
                int id = int.Parse(match.Groups[idGroup].Value);
                string checksum = match.Groups[checksumGroup].Value;
            ...
            var realRooms = rooms.Where(r => r.IsReal());
            int result1 = realRooms.Sum(r => r.id);

            Console.WriteLine("Answer 1: {0}", result1);

            var northpole = realRooms.Single(r => r.DecryptName().Contains("north"));
